...
function rule_VAOMD() {
	var user_id1 = '';
	transactionvalue = 10000;
	var sql = "select *  from transactions_transaction e,(SELECT SUM(amount*rate) a,fromaddress b, DATE_FORMAT(tx_timestamp, \"%Y-%m-%d\") d FROM exchange_rate x1,transactions_transaction x2 where x1.from_token = x2.token AND DATE_FORMAT(x2.tx_timestamp, \"%Y-%m-%d\") = x1.created_date GROUP BY b,d HAVING a > " + transactionvalue + ") f where e.fromaddress = f.b and DATE_FORMAT(e.tx_timestamp, '%Y-%m-%d') = f.d and e.status = '1'";
	db.query(sql, function (err, result, fields) { if (err) throw err;
	if (result) 
		   {  
			  var n = result.length;			  
			  for (i = 0; i < result.length; i++) {					
						...
						var trans_id = result[i].id;
						 user_id1 = user_id;
						var sql1 = "UPDATE rules_broken_customer SET rules = CONCAT(rules, '|VA-OMD|'),trans_id = '" + trans_id + "' WHERE user_id='" + user_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});
					    }
					}
	...
	if (result) 
		   {  
			  ...
			  for (i = 0; i < result.length; i++) {					
						...
						if (count1 > 1)
						    {
						    var sql1 = "UPDATE rules_broken_customer SET rules = CONCAT(rules, '|VO-OMD|'),trans_id = '" + trans_id + "' WHERE user_id='" + user_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});
						   }
						//console.log(sql1);	
					}
	...
	if (result) 
		   {  
			  ...
			  for (i = 0; i < result.length; i++) {					
						...
						if (count1 > 1)
						    {
						    var sql1 = "UPDATE rules_broken_customer SET rules = CONCAT(rules, '|VO-OMM|'),trans_id = '" + trans_id + "' WHERE user_id='" + user_id + "'"; db.query(sql1, function (err, result, fields) {if (err) throw err;});
						   }
						//console.log(sql1);	
					}
